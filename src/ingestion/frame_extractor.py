...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, default="0", help="Webcam index (int) or video file path (str)")
    ...
    print(f"FrameExtractor Ingestion CLI started. Input: {source_val}, Output FPS: {args.output_fps}")
    try:
        for frame in cap.stream():
            ...
            display_frame = cv2.cvtColor((processed * 255.0).astype("uint8"), cv2.COLOR_RGB2BGR)
            try:
